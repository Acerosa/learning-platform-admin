...
const viewRows: Record<string, readonly Record<string, unknown>[]> = {
  ...
  recent_attempts: [{ attempt_id: "attempt-1", student_number: "S-1", activity_key: "activity-a", activity_version: "1.0.0", status: "completed", score: 8, max_score: 10, completed_at: "2026-08-11T00:01:00Z" }],
  ...
  diagnostic_sessions: [{ session_id: "diag-1", student_name: "Alex Rivera", student_id: "STU1001", hub_code: "level-3-it-year-1-readiness", hub_name: "Level 3 IT Year 1 Readiness Hub", course_key: "ocr-level-3-it", course_title: "OCR Level 3 IT", diagnostic_key: "level-3-it-year-1-readiness", diagnostic_version: "1.0.0", status: "completed", started_at: "2026-09-02T09:00:00Z", completed_at: "2026-09-02T09:22:00Z", response_count: 1, not_sure_count: 0 }],
...
test("live service reads every MVP surface through admin_api and maps safe rows", async () => {
  ...
  assert.equal(data.diagnosticSessions[0].diagnosticVersion, "1.0.0");
...
test("listRecentAttempts reads the bounded admin_api recent_attempts view", async () => {
  const fake = fakeClient();
  const service = createSupabaseAdminReadService(fake.client);
  const recent = await service.listRecentAttempts();
  assert.equal(recent.length, 1);
  assert.equal(recent[0].attemptId, "attempt-1");
  assert.equal(recent[0].learnerNumber, "S-1");
  assert.match(fake.selections.join("\n"), /recent_attempts:attempt_id,student_number,activity_key,activity_version,status,score,max_score,completed_at/);
});
