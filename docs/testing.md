...
- **Hub registration:** manifest import, form validation, diagnostics, demo snapshot refresh, live `admin_api.register_hub` / `admin_api.update_hub` arguments, duplicate-error mapping, publication linkage and health reporting.
- **Curriculum authoring:** factories, stable IDs, JSON/Excel import, sanitisation, drafts, preview, export, publication lifecycle, compare, restore, platform publication payload and shared content-engine interoperability.
